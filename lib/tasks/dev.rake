namespace :dev do
  ...
  task setup: :environment do
    if Rails.env.development?
      ...
      show_spinner("Creating default user...") { %x(rake dev:add_default_user) }
      show_spinner("Populating brazilian states...") { %x(rails db:seed) }
      show_spinner("Populating other tables using fakes...") { %x(rake dev:faker_populate) }
    else
  ...
  task faker_populate: :environment do
    20.times do |i|
      ...
      proponent = Proponent.create!(
        ...
        birth_date: Faker::Date.between(from: Date.today.ago(75.years), to: Date.today.ago(16.years)),
        salary: salary,
